...
ntl_producer_means <- ntl_producers %>%
  group_by(year, plot, taxon_name) %>%
  summarise(mean_abundance = sqrt(mean(abundance, na.rm = TRUE)), .groups = "drop") %>%
  mutate(mean_abundance = ifelse(is.na(mean_abundance), 0, mean_abundance))


ntl_consumer_means <- ntl_consumer_means %>%
  pivot_wider(names_from = taxon_name, values_from = mean_abundance, values_fill = list(mean_abundance = 0))
ntl_producer_means <- ntl_producer_means %>%
  pivot_wider(names_from = taxon_name, values_from = mean_abundance, values_fill = list(mean_abundance = 0))
...
ntl_consumer_matrix = ntl_consumer_means %>% 
  column_to_rownames(var = "site.year")

ntl_producer_matrix = ntl_producer_means %>% 
  column_to_rownames(var = "site.year")

ntl_consumer_matrix = ntl_consumer_matrix %>%
  dplyr::select(-plot, -year)

ntl_producer_matrix = ntl_producer_matrix %>%
  dplyr::select(-plot, -year)

# NTL NMDS ----

#doing this as a combined coral/algae community and as just coral or alage cover
nmds_consumer <- metaMDS(ntl_consumer_matrix)
nmds_producer <- metaMDS(ntl_producer_matrix)
...
nmds_consumer_data <- data.frame(nmds_consumer_coords, site.year = rownames(ntl_consumer_matrix))
nmds_prooducer_data <- data.frame(nmds_producer_coords, site.year = rownames(ntl_producer_matrix))
...
# consumers
plot_nmds_consumer <- ggplot(ntl_consumer_means, aes(x = MDS1,
                                                     y = MDS2,
                                                     color = plot,
                                                     label = year)) +
  geom_point(size = 3) +
...
# producers
plot_nmds_producer <- ggplot(ntl_producer_means, aes(x = MDS1,
                                                     y = MDS2,
                                                     color = plot,
                                                     label = year)) +
  geom_point(size = 3) +
  geom_text(check_overlap = TRUE, size = 3, color = "black", vjust = 1.5) +
  labs(title = "NTL Producer Community by Site", x = "NMDS1", y = "NMDS2") +
  scale_color_manual(values = c("#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd",
                                "#8c564b", "#e377c2", "#7f7f7f", "#bcbd22", "#17becf",
...
ggsave(filename = paste0("figures/NMDS/NTL", "/", "plot_nmds_producer", ".png"),
...
#consumers
D <- vegan::vegdist(ntl_consumer_matrix,"bray")
...
trajectoryPCoA(D, sites = ntl_consumer_means$plot, surveys = ntl_consumer_means$vector_nums, traj.colors = custom_palette, lwd = 1, survey.labels = T) 
...
png(filename = "figures/CTA/NTL/ntl_consumer_trajectory_plot.png", width = 12, height = 8, res = 300, units = "in")
trajectory_plot
dev.off()

# producers
Dd <- vegan::vegdist(ntl_producer_matrix,"bray")

ntl_producer_means = ntl_producer_means %>% 
  mutate(vector_nums = year - min(year) + 1)

trajectoryPCoA(Dd, sites = ntl_producer_means$plot, surveys = ntl_producer_means$vector_nums, traj.colors = custom_palette, lwd = 2, survey.labels = T) 
legend(ncol = 1, y.intersp = 0.5, "topright", inset = c(0.02, 0), bty="n", legend=c("001d", "004b", "n01b", "n04d", "002d", "004f", "002c", "0sub", "020b", "0spb", "n20a", "n20b", "n01a", "n04a"),  col = c( "#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd",
                                                                                                                                                                                                               "#8c564b", "#e377c2", "#7f7f7f", "#bcbd22", "#17becf",
                                                                                                                                                                                                               "#aec7e8", "#ffbb78", "#98df8a", "#ff9896"), lwd=2)
...
# Save the plot as a PNG file
png(filename = "figures/CTA/NTL/ntl_producer_trajectory_plot.png", width = 13, height = 8, res = 300, units = "in")
trajectory_plot
dev.off()
...
unique_sites <- unique(ntl_consumers$plot)
...
for (site in unique_sites) {
  ...
  csv_filename <- paste0("tables/NTL/sites/ntl_consumer_segment_lengths_", site, ".csv")
  write.csv(segment_lengths_df, csv_filename, row.names = FALSE)
  
  # Generate and save the trajectory plot
  plot_filename <- paste0("figures/CTA/NTL/ntl_consumer_trajectory_plot_", site, ".png")
...
#producers
unique_sites_producers <- unique(ntl_consumers$plot)
...
for (site in unique_sites_producers) {
  ...
  # Save segment lengths as a CSV file
  csv_filename <- paste0("tables/NTL/sites/ntl_producer_segment_lengths_", site, ".csv")
  write.csv(segment_lengths_df, csv_filename, row.names = FALSE)
  
  # Open the PNG file for saving
  filename <- paste0("figures/CTA/NTL/ntl_producer_trajectory_plot_", site, ".png")
...
consumer_lengths <- trajectoryLengths(D, sites = ntl_consumer_means$plot, surveys = ntl_consumer_means$vector_nums)
consumer_lengths <- as.data.frame(consumer_lengths)
consumer_lengths$site <- site_order  
write.csv(consumer_lengths, "tables/NTL/consumer/consumer_lengths.csv", row.names = FALSE)

# Calculate trajectory lengths (relative to initial)
consumer_lengths_initial <- trajectoryLengths(D, sites = ntl_consumer_means$plot, surveys = ntl_consumer_means$vector_nums, relativeToInitial = TRUE)
consumer_lengths_initial <- as.data.frame(consumer_lengths_initial)
consumer_lengths_initial$site <- site_order
write.csv(consumer_lengths_initial, "tables/NTL/consumer/consumer_lengths_initial.csv", row.names = FALSE)

# Calculate trajectory angles (not relative to initial)
consumer_angles <- trajectoryAngles(D, sites = ntl_consumer_means$plot, surveys = ntl_consumer_means$vector_nums)
consumer_angles <- as.data.frame(consumer_angles)
consumer_angles$site <- site_order
write.csv(consumer_angles, "tables/NTL/consumer/consumer_angles.csv", row.names = FALSE)

# Calculate trajectory angles (relative to initial)
consumer_angles_initial <- trajectoryAngles(D, sites = ntl_consumer_means$plot, surveys = ntl_consumer_means$vector_nums, relativeToInitial = TRUE)
consumer_angles_initial <- as.data.frame(consumer_angles_initial)
consumer_angles_initial$site <- site_order
write.csv(consumer_angles_initial, "tables/NTL/consumer/consumer_angles_initial.csv", row.names = FALSE)

# # Calculate trajectory distances
# consumer_trajectory_distances <- trajectoryDistances(D, sites = ntl_consumer_means$plot, surveys = ntl_consumer_means$vector_nums)
# consumer_trajectory_distances <- as.data.frame(consumer_trajectory_distances)
# consumer_trajectory_distances$site <- site_order
# write.csv(consumer_trajectory_distances, "tables/NTL/consumer/consumer_trajectory_distances.csv", row.names = FALSE)

# Calculate trajectory convergence
consumer_convergence <- trajectoryConvergence(D, sites = ntl_consumer_means$plot, surveys = ntl_consumer_means$vector_nums)
consumer_convergence <- as.data.frame(consumer_convergence)
consumer_convergence$site <- site_order
write.csv(consumer_convergence, "tables/NTL/consumer/consumer_convergence.csv", row.names = FALSE)

# Calculate trajectory directionality
consumer_directionality <- trajectoryDirectionality(D, sites = ntl_consumer_means$plot, surveys = ntl_consumer_means$vector_nums)
consumer_directionality <- as.data.frame(consumer_directionality)
consumer_directionality$site <- site_order
write.csv(consumer_directionality, "tables/NTL/consumer/consumer_directionality.csv", row.names = FALSE)

# Extract trajectory metrics for producers
# Calculate trajectory lengths (not relative to initial)
producer_lengths <- trajectoryLengths(Dd, sites = ntl_producer_means$plot, surveys = ntl_producer_means$vector_nums)
producer_lengths <- as.data.frame(producer_lengths)
producer_lengths$site <- site_order
write.csv(producer_lengths, "tables/NTL/producer/producer_lengths.csv", row.names = FALSE)

# Calculate trajectory lengths (relative to initial)
producer_lengths_initial <- trajectoryLengths(Dd, sites = ntl_producer_means$plot, surveys = ntl_producer_means$vector_nums, relativeToInitial = TRUE)
producer_lengths_initial <- as.data.frame(producer_lengths_initial)
producer_lengths_initial$site <- site_order
write.csv(producer_lengths_initial, "tables/NTL/producer/producer_lengths_initial.csv", row.names = FALSE)

# Calculate trajectory angles (not relative to initial)
producer_angles <- trajectoryAngles(Dd, sites = ntl_producer_means$plot, surveys = ntl_producer_means$vector_nums)
producer_angles <- as.data.frame(producer_angles)
producer_angles$site <- site_order
write.csv(producer_angles, "tables/NTL/producer/producer_angles.csv", row.names = FALSE)

# Calculate trajectory angles (relative to initial)
producer_angles_initial <- trajectoryAngles(Dd, sites = ntl_producer_means$plot, surveys = ntl_producer_means$vector_nums, relativeToInitial = TRUE)
producer_angles_initial <- as.data.frame(producer_angles_initial)
producer_angles_initial$site <- site_order
write.csv(producer_angles_initial, "tables/NTL/producer/producer_angles_initial.csv", row.names = FALSE)

# # Calculate trajectory distances
# producer_trajectory_distances <- trajectoryDistances(Dd, sites = ntl_producer_means$plot, surveys = ntl_producer_means$vector_nums)
# producer_trajectory_distances <- as.data.frame(producer_trajectory_distances)
# producer_trajectory_distances$site <- site_order
# write.csv(producer_trajectory_distances, "tables/NTL/producer/producer_trajectory_distances.csv", row.names = FALSE)

# Calculate trajectory convergence
producer_convergence <- trajectoryConvergence(Dd, sites = ntl_producer_means$plot, surveys = ntl_producer_means$vector_nums)
producer_convergence <- as.data.frame(producer_convergence)
producer_convergence$site <- site_order
write.csv(producer_convergence, "tables/NTL/producer/producer_convergence.csv", row.names = FALSE)

# Calculate trajectory directionality
producer_directionality <- trajectoryDirectionality(Dd, sites = ntl_producer_means$plot, surveys = ntl_producer_means$vector_nums)
producer_directionality <- as.data.frame(producer_directionality)
producer_directionality$site <- site_order
write.csv(producer_directionality, "tables/NTL/producer/producer_directionality.csv", row.names = FALSE)
