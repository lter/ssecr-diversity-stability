...
drive_download(drive_folder[drive_folder$name == 'combined_agg_stability.csv',], path = tmp, overwrite = TRUE)
comb <- read.csv(tmp)
...
##### terrestrial shannon ##### The relationship is stronger for richness. 
...
(aquatic_prod_prod_plot <- 
    ggplot() +
    geom_point(data = aquatic_comb, aes(x = prod_richness, y = prod_stability , colour = site)) +
    stat_smooth(data = aquatic_comb, aes(x =  prod_richness, y = prod_stability, colour = site),
                method = "glm", se = F) +
    # stat_smooth(data = aquatic_comb, aes(x =  prod_richness, y = prod_stability),
    #             method = "glm", se = T, colour = "black") +
    theme_classic() 
)
