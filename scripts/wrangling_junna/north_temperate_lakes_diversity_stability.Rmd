...
```{r phytoplankton in trout lake area, fig.width=10, fig.height=4}
library(tidyverse)
library(vegan)
rm(list=ls())
##
...
data_phyto_trout <- data_phyto_trout %>% group_by(year, month, day, unique_ID, species) %>% summarise(abundance = sum(abundance))
##
## convert it into a data.frame
data_phyto_trout_convert <- data_phyto_trout %>% mutate(date=as.Date(year, month, day)) %>% pivot_wider(names_from=species, values_from = abundance, values_fill = 0)
...
###
stability_nep <- productivity %>% group_by(lake) %>% summarise(cv_1 = 1.0/sd(Fnep))
...
p2 <- ggplot(data=diversity_stability_phyto, aes(x=richness, y=cv_1)) +
  ...
  geom_smooth(method='lm') +
  stat_cor(color='green') +
  labs(x='Producer richness', y='Producer aggregated stability (1/cv)', title='Producer (phytoplankton)')
...
```{r zooplankton trout lake, fig.width=10, fig.height=4}
#
...
data_zoop_trout <- data_zoop_trout[data_zoop_trout$species!="", ]
#
sum(duplicated(data_zoop_trout[,1:14])==TRUE)
## 15 duplicated rows
#
## remove duplicated rows
data_zoop_trout <- data_zoop_trout %>% group_by(year, month, day, unique_ID, species) %>% summarise(abundance = sum(abundance))
##
## convert it into a data.frame
data_zoop_trout_convert <- data_zoop_trout %>% mutate(date=as.Date(year, month, day)) %>% pivot_wider(names_from=species, values_from = abundance, values_fill = 0)
## change NA values into 0
...
stability_cv <- data_zoop_trout_annual %>% group_by(unique_ID) %>% summarise(cv_1 = 1.0/sd(total))
##
## further step to site level data
...
data_binvert_trout <- data_binvert_trout[data_binvert_trout$species!="", ]
#
sum(duplicated(data_binvert_trout[,1:12])==TRUE)
## 0 duplicated rows
##
## convert it into a data.frame: this is a yearly dataset
data_binvert_trout_convert <- data_binvert_trout %>% pivot_wider(names_from=species, values_from = abundance, values_fill = 0)
## change NA values into 0
data_binvert_trout_convert[is.na(data_binvert_trout_convert)] <- 0.0
##
## aggregated into annual average: no need 
data_binvert_trout_annual <- data_binvert_trout_convert %>% ungroup %>% dplyr::select(-c(site, taxa_type, ecosystem, habitat, biome, guild, plot, unit_abundance, scale_abundance)) 
##
## calculate stability
data_binvert_trout_annual$total <- rowSums(data_binvert_trout_annual[, -(1:2)])
stability_cv <- data_binvert_trout_annual %>% group_by(unique_ID) %>% summarise(cv_1 = 1.0/sd(total))
##
## further step to site level data
data_binvert_trout_site <- data_binvert_trout_annual %>% ungroup %>% dplyr::select(-year) %>% group_by(unique_ID) %>% summarise_all(mean)
H <- diversity(data_binvert_trout_site[,-1])
S <- specnumber(data_binvert_trout_site[,-1])
## they are highly correlated. 
plot(H, S)
##
