...
# if you want to see results of terrestrial ecosystems; uncomment the next line. 
# mar <- terr

# add new columns, do log transformation
mar$site_habitat_fine <- paste(mar$site_new, mar$habitat_fine, sep='_')
...
mar_site_mean <- mar %>% group_by(site_new) %>% 
...
if (! "prod_richness_zscore_site_mean" %in% names(mar)) {
  mar <- mar %>% left_join(mar_site_mean, by="site_new")
}
...
#
mar$prod_richness_log_zscore_devi <- mar$prod_richness_log_zscore - mar$prod_richness_log_zscore_site_mean
mar$con_richness_log_zscore_devi <- mar$con_richness_log_zscore - mar$con_richness_log_zscore_site_mean
mar$prod_stability_log_zscore_devi <- mar$prod_stability_log_zscore - mar$prod_stability_log_zscore_site_mean
mar$con_stability_log_zscore_devi <- mar$con_stability_log_zscore - mar$con_stability_log_zscore_site_mean
...
#-------------------marine producer community
# use model D -- group mean centered path model in Byrnes and Dee, 2025
mod_mar_prod1 <- lmer(data = mar, prod_stability_log_zscore ~ prod_richness_zscore_devi + con_richness_zscore_devi + prod_richness_zscore_site_mean + con_richness_zscore_site_mean + (1|site_new))
...
# use model D
mod_mar_con1 <- lmer(data = mar, con_stability_log_zscore ~ prod_richness_zscore_devi + con_richness_zscore_devi + prod_richness_zscore_site_mean + con_richness_zscore_site_mean + (1|site_new))
...
# try to specify non-informative prior and see if the results are different. Junna, Nov 8, 2025
...
for (isite in 1:length(sites)) {
  ...
  period <- data.frame(start = seq(1, nyears - duration + 1, by = 2), end = seq(duration, nyears, by = 2))
...
# when adding residual autocorrelation structure, diversity's effects are mostly non-significant. 
